  ...
  return (
    <div className={"flex-1 flex flex-col min-h-0 h-full overflow-hidden " + themeBg + " " + fontClass}>
      ...
      {examMode === 'strict' ? (
        ...
        /* Standard Header in Practice Mode (Responsive 2-row on Mobile, 1-row on Desktop) */
        <div className="bg-white border-b border-slate-200 px-2.5 sm:px-6 py-2 flex flex-col md:flex-row md:items-center md:justify-between gap-1.5 sm:gap-2 shrink-0 shadow-2xs">
          
          {/* ROW 1 ON MOBILE / LEFT ON DESKTOP: Test Info & Mode Pill */}
            ...
            <div className="flex items-center space-x-2 sm:space-x-3 min-w-0">
              ...
              <div className="min-w-0">
                <div className="flex items-center space-x-1.5">
                  <h2 className="font-bold text-slate-900 text-xs sm:text-sm truncate max-w-[170px] xs:max-w-[220px] sm:max-w-none">
                    {currentTest.title}
                  </h2>
                  <span className="hidden lg:inline-block px-2 py-0.5 rounded-full bg-emerald-50 text-emerald-700 text-[10px] font-bold border border-emerald-200 shrink-0">
                    Cambridge Official
                  </span>
                </div>
                <div className="flex items-center space-x-1.5 text-[10px] sm:text-[11px] text-slate-500 truncate">
                  <span>{currentTest.totalQuestions} câu</span>
                  <span>•</span>
                  <span className="text-indigo-600 font-semibold">Đã làm: {exam.answeredCount}/{currentTest.totalQuestions}</span>
                </div>
              </div>
            </div>

            {/* Mode Switcher on Mobile (compact) */}
            <div className="flex md:hidden items-center bg-slate-100 p-0.5 rounded-lg border border-slate-200 text-[11px] font-semibold shrink-0">
              <button
          ...
          {/* ROW 2 ON MOBILE / RIGHT ON DESKTOP: Touch-scrollable Action Buttons */}
          <div className="flex items-center space-x-1.5 sm:space-x-2 overflow-x-auto no-scrollbar py-0.5 shrink-0 max-w-full">
            {/* Quick Test Switcher Dropdown (Desktop only) */}
            {allListeningTests.length > 1 && (
              <select
                ...
                className="hidden lg:block bg-white border border-slate-200 text-xs font-bold text-slate-700 px-2 py-1 rounded-lg focus:outline-none focus:ring-1 focus:ring-purple-500 max-w-[160px] truncate cursor-pointer"
            ...
            <div className="hidden md:flex items-center bg-slate-100 p-0.5 rounded-lg border border-slate-200 text-xs font-semibold shrink-0">
              <button
                onClick={() => setExamMode('practice')}
