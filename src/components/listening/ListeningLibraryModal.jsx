...
export default function ListeningLibraryModal({
  ...
  const handleToggleHideMastered = () => {
    setHideMastered(prev => {
      const next = !prev;
      try {
        localStorage.setItem('ielts_listening_hide_mastered', String(next));
      } catch {}
      return next;
    });
  };
  ...
  const filteredTests = useMemo(() => {
    return allListeningTests.filter(test => {
      ...
      // Hide mastered if checked (unless on mastered tab)
      if (hideMastered && activeTab !== 'mastered' && isMastered) {
        return false;
      }
        ...
        <div className="px-5 py-3 border-b border-slate-200 bg-white flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex items-center space-x-1 bg-slate-100 p-1 rounded-xl text-xs font-bold text-slate-600 overflow-x-auto scrollbar-none max-w-full">
            ...
            <button
              ...
              className={`px-2.5 py-1.5 rounded-lg transition-all cursor-pointer shrink-0 flex items-center space-x-1 ${
                activeTab === 'mastered' ? 'bg-white text-emerald-800 shadow-2xs font-black' : 'hover:text-slate-900'
              }`}
            >
          ...
          <div className="flex items-center space-x-2 w-full sm:w-auto">
            {/* Ẩn đề đã thuộc Checkbox */}
            <label className="flex items-center space-x-1.5 text-xs text-slate-600 cursor-pointer select-none px-2 py-1 rounded-lg hover:bg-slate-100 transition-colors shrink-0">
              <input
                type="checkbox"
